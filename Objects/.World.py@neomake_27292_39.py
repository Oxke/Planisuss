...
class World:
    ...
    def get_adjacent(self, cell: Cell):
        ...
        i'm using this other metric, where the distance is the minimum number of
        steps to connect two cells passing only through adjacent cells. This is
        a metric that is more similar to the one used in the game of life and I
        might consider in future including it as on option, since I just need to
        changer the get_neighbors function. In particular the continent Q"""
        adjacent_cells = []
        if cell.x > 0:
