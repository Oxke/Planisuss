...
class World:
        ...
        while len(pangea) > 0:
            ...
            for neighbor in self.get_adjacent(cell):
                ...
                if rd.random() < p:
                    ...
                    neighbor.water = True
        # self.grid[x][y].add_herb(None, self)
    ...
    def plot(self, title="Planisuss"):
        ...
        plt.imshow(np.dstack((np.zeros((self.num_cells, self.num_cells)),
                              vegetob, water)))
        # names the axes accordigly to the number of cells and adds some
        # explaining text
        plt.xlabel("x")
        plt.ylabel("y")
        plt.text(0, -1, "x: 0 to {}".format(self.num_cells-1))
        plt.text(-1, 0, "y: 0 to {}".format(self.num_cells-1))
        plt.show()
